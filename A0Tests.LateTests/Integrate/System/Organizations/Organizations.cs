...
namespace A0Tests.LateTests.Integrate.Sys
{
    ...
    public class Test_IA0SysOrganizationsRepo : Test_Base
    {
        ...
        {
            // Поля для запроса.
            dynamic fields = this.Repo.TreeID.GetFieldRequest();
            fields.Add("Mark");
            fields.Add("TreeName");
            fields.Add("CreateMoment");
            fields.Add("UpdateMoment");

            dynamic iter = this.Repo.TreeID.Read(fields, null, null);
            Assert.NotNull(iter);
            ...
            while (!iter.EOF)
            {
                dynamic currentItem = iter.Current;
        ...
        public void Test_TreeNodeID()
        {
            ...
            dynamic iterTree = this.Repo.TreeID.Read(null, null, null);
            Assert.NotNull(iterTree);
            Assert.NotZero(iterTree.Count, "Ожидается наличие структур организаций в БД");

            while (!iterTree.EOF)
            {
                dynamic treeFields = iterTree.Current;
                int treeID = treeFields.Value["TreeID"];

                // Итератор по узлам.
                dynamic iterNode = this.Repo.TreeNodeID.Read(treeID, fields, null, null);

                int count = iterNode.Count;

                while (!iterNode.EOF)
                {
                    dynamic nodeFields = iterNode.Current;
                    ...
                    dynamic nodeType = nodeFields.Value["NodeType"];
        ...
        {
            // Поля для запроса.
            dynamic fields = this.Repo.ExecutorID.GetFieldRequest();
            fields.Add("INN");
            fields.Add("OKPO");
            fields.Add("Shifr");
            fields.Add("FullName");
            ...
            dynamic iterExecutor = this.Repo.ExecutorID.Read(fields, null, null);

            int count = iterExecutor.Count;

            while (!iterExecutor.EOF)
            {
                dynamic executorFields = iterExecutor.Current;
        ...
        {
            // Поля для запроса.
            dynamic fields = this.Repo.EmployeeID.GetFieldRequest();
            fields.Add("FIO");
            fields.Add("POST");
            fields.Add("Document");

            // Итератор	по сотрудникам.
            dynamic iterEmployee = this.Repo.EmployeeID.Read(fields, null, null);

            int count = iterEmployee.Count;

            while (!iterEmployee.EOF)
            {
                dynamic employeeFields = iterEmployee.Current;
        ...
        {
            // Поля для запроса
            dynamic reqFieldsTree = this.Repo.TreeID.GetFieldRequest();
            ...
            // Итератор по структурам организаций.
            dynamic iterTree = this.Repo.TreeID.Read(reqFieldsTree, null, null);
            Assert.NotNull(iterTree);
            Assert.NotZero(iterTree.Count, "Ожидается наличие структур организаций в БД");
            ...
            while (!iterTree.EOF)
            {
                ...
                dynamic tree = this.Repo.Tree.Read(treeID, 0); // 0 - EAccessKind.akRead
                Assert.NotNull(tree);
                ...
                dynamic headNode = tree.HeadNode;
                Assert.NotNull(headNode);

                // Загрузка узлов.
                // Поля для запроса узлов.
                dynamic fields = this.Repo.TreeNodeID.GetFieldRequest();
                fields.Add("NodeMark");
                fields.Add("NodeName");
                fields.Add("NodeType");

                dynamic order = this.Repo.TreeNodeID.GetOrderRequest();
                order.Clear();
                order.Add("[ExecutorsTreeNode].NodeID");

                dynamic where = this.Repo.TreeNodeID.GetWhereRequest();
                ...
                // Итератор по узлам.
                dynamic iterNode = this.Repo.TreeNodeID.Read(treeID, fields, where, order);

                Assert.AreEqual(headNode.Nodes.Count, iterNode.Count);

                while (!iterNode.EOF)
                {
                    dynamic fieldsNode = iterNode.Current;
