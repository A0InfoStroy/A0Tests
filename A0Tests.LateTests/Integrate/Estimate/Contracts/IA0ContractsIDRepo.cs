// $Date: 2020-12-25 16:32:54 +0300 (Пт, 25 дек 2020) $
// $Revision: 472 $
// $Author: agalkin $
...
namespace A0Tests.LateTests.Integrate.Estimate
{
    ...
    public class Test_IA0ContractsIDRepo : Test_Estimate
    {
        ...
        {
            dynamic iter = this.ContractRepo.Read(this.GetProjGUID(), null, null, null);
        ...
        {
            dynamic appFieldsReq = this.ContractRepo.GetFieldRequest();
            appFieldsReq.Add("[Contracts].[Number]");
            dynamic iter = this.ContractRepo.Read(this.GetProjGUID(), appFieldsReq, null, null);
            this.CheckNonEmptyIterator(iter);
            while (iter.Next())
            {
        ...
        public void Test_ReadErrorFields()
        {
            ...
            try
            {
                dynamic iterator = this.ContractRepo.Read(this.GetProjGUID(), appFieldsReq, null, null);
        ...
        {
            int projectID = 133;
            dynamic whereReq = this.ContractRepo.GetWhereRequest();
            whereReq.Node.And3("[Contracts].[ProjID]", "=", projectID.ToString());
            dynamic iter = this.ContractRepo.Read(this.GetProjGUID(), null, whereReq, null);
            this.CheckNonEmptyIterator(iter);
            bool exist = false;
            while (iter.Next())
            {
        ...
        public void Test_ReadErrorIDWhere()
        {
            ...
            try
            {
                dynamic iterator = this.ContractRepo.Read(this.GetProjGUID(), null, whereReq, null);
                result = iterator.Next();
            }
            catch (System.Runtime.InteropServices.COMException ex)
            {
        ...
        {
            dynamic orderReq = this.ContractRepo.GetOrderRequest();
            orderReq.Add("[Contracts].[ContractID]");
            dynamic iter = this.ContractRepo.Read(this.GetProjGUID(), null, null, orderReq);
            this.CheckNonEmptyIterator(iter);
            bool exist = false;
            while (iter.Next())
            {
        ...
        public void Test_ReadErrorOrder()
        {
            ...
            try
            {
                dynamic iterator = this.ContractRepo.Read(this.GetProjGUID(), null, null, orderReq);
                result = iterator.Next();
            }
            catch (System.Runtime.InteropServices.COMException ex)
            {
        ...
        {
            dynamic iter = this.ContractRepo.Read(Guid.Empty, null, null, null);
        ...
        private void CheckNonEmptyIterator(dynamic iter)
        {
            Assert.NotNull(iter);
